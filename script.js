...
.then(dataProductos => {
    dataProductos.forEach((productoEnArray, indice) => {
        ...
        <div class="card border-dark mb-3 id="producto${indice}" style="max-width: 20rem; margin:6px">
            <img src="../img/${productoEnArray.img}" class="card-img-top" alt="...">
            <div class="card-header">${productoEnArray.nombre}</div>
            <div class="card-body">
                <p class="card-text">${productoEnArray.descripcion}</p>
    ...
    dataProductos.forEach((productoEnArray, indice) =>{
        document.getElementById(`boton${indice}`).addEventListener('click', () => {
            if(productos.find(producto => producto.nombre == productoEnArray.nombre)) {
                ...
                let nuevoProducto = new Producto(productoEnArray.nombre, productoEnArray.descripcion, productoEnArray.precio, productoEnArray.img)
...
botonFinalizarCompra.addEventListener('click', () => {
    localStorage.setItem('carrito', JSON.stringify([]))
    alert("Gracias por su compra! Estamos preparando su pedido.");
})
